...
import { BadgeCheck, Copy, Heart, LoaderCircle, RotateCw, Share, Sparkles, Trash, X } from 'lucide-react'
...
const SentenceWriterPage = () => {
    ...
    const rewriteSentence = async() => {
        try{
            ...
            if (!data?.data?.success) return setError(true) || errorShow(data?.data?.message)
    ...
    <div className='p-5 md:p-0'>
        ...
        <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                {/* share section */}
           {
                ...
                <div className='fixed top-0 flex gap-5 flex-col-reverse justify-center items-center right-0 w-full h-screen bg-black/10 z-20'>
                    <div onClick={() => setShare(false)} className='px-4 py-2 rounded-md flex justify-center items-center gap-2 bg-red-500 text-white transition-all duration-300 hover:scale-105 active:scale-95 cursor-pointer'>
                    <X size={18} />
                    <span>
                        Cancel
                    </span>
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                        <button onClick={() => shareOnLinkedin(copied)} className="group flex items-center justify-center gap-3 rounded-xl bg-[#0A66C2] px-5 py-4 text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:scale-105 hover:shadow-2xl active:scale-95">
                        ...
                        <button onClick={() => shareOnWhatsapp(copied)} className="group flex items-center justify-center gap-3 rounded-xl bg-[#25D366] px-5 py-4 text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:scale-105 hover:shadow-2xl active:scale-95">
                        ...
                        <button onClick={() => shareTwitter(copied)} className="group flex items-center justify-center gap-3 rounded-xl bg-black px-5 py-4 text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:scale-105 hover:shadow-2xl active:scale-95">
        ...
        {rewrite?.map((item, index) => (
            ...
            <div className="mt-6 flex items-center justify-between gap-3 border-t pt-4">
                ...
                <button onClick={() => {setShare(true), setCopied(item?.sentence)}} className="flex items-center gap-2 rounded-lg bg-linear-to-r from-indigo-500 to-purple-600 px-4 py-2 text-sm font-medium text-white shadow transition hover:scale-105 cursor-pointer">
                    <Share size={15} />
                    <span>
                    Share
                    </span>
