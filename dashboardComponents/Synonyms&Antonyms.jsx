...
const SynonymsAntonymsPage = () => {
  ...
  const copySynandAnt = (word, type) => {
    type === "syn" ? setSynCopied(true) : setAntCopied(true);

    const text = word.map((words) => words).join(", ");

    window.navigator.clipboard.writeText(text);
    setTimeout(
      () => (type === "syn" ? setSynCopied(false) : setAntCopied(false)),
      1000,
    );
  };
    ...
    <div className="w-full min-h-screen bg-white pb-5">
      <nav className="flex justify-between items-center h-16 bg-white shadow px-5">
        <div className="flex justify-center items-center gap-3">
          ...
          <span className="font-semibold text-2xl">Synonyms & Antonyms</span>
        </div>
        <button
          onClick={removeWord}
          className="px-4 py-2 rounded-md bg-red-500 text-white flex justify-center items-center gap-2 font-semibold hover:bg-red-400 transition-all duration-300 cursor-pointer hover:scale-105 active:scale-100">
          <Trash size={18} />
          <span>Clean</span>
        </button>
      </nav>
      ...
      <div className="flex justify-center items-center m-10 ">
        ...
        />
        <button
          disabled={loader || !word?.trim()}
          onClick={searchWord}
          className="px-5 py-3 bg-blue-600 text-white rounded-r-lg cursor-pointer tracking-wider disabled:cursor-not-allowed">
          {loader ? (
      ...
      {synData?.word ? (
        <div className="max-w-5xl mx-auto rounded-2xl border bg-white dark:bg-slate-900 dark:border-slate-800 shadow-sm ">
          ...
          <div className="flex items-center gap-5 mb-6 border-b h-25 px-8">
            <div
              onClick={() => textToSpeech(synData?.word)}
              className="bg-gray-100 w-15 h-15 flex justify-center items-center rounded-lg border border-gray-200 cursor-pointer">
              {speechLoader ? (
          ...
          <div className="grid md:grid-cols-2 gap-8 p-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  onClick={() => copySynandAnt(synData?.synonyms, "syn")}
                  className="w-full inline-flex mt-5 text-green-500 border border-green-500 px-4 py-2 rounded-md justify-center items-center gap-2 cursor-pointer">
                  {synCopied ? (
                    <Check />
                  ) : (
                    <>
                      <Copy size={15} />
                      <span>Copy Synonyms</span>
                    </>
                  )}
                </button>
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  onClick={() => copySynandAnt(synData?.antonyms)}
                  className="w-full inline-flex mt-5 text-red-500 border border-red-500 px-4 py-2 rounded-md justify-center items-center gap-2 cursor-pointer">
                  {antCopied ? (
                    <Check />
                  ) : (
                    <>
                      <Copy size={15} />
                      <span>Copy Antonyms</span>
                    </>
                  )}
                </button>
              </div>
          ...
          <div className="mt-10 border-t pt-6 dark:border-slate-700 p-5 flex justify-between items-center">
            ...
            <button
              onClick={() => copyText(synData?.example)}
              className="px-4 py-2 flex justify-center items-center gap-2 bg-white text-indigo-500 shadow rounded-sm text-sm cursor-pointer">
              {copied ? (
                <Check />
              ) : (
                <>
