...
const ExplainGrammerMistakesPage = () => {
  ...
  const { explainGrammer, setExplainGrammer, setExplainGrammerUpdate } =
    everyWearCon();
  ...
  const explainSentence = async () => {
    try {
      ...
      const { data } = await api.post("/groq/explainGrammarMistakes", {
        sentence,
      });
    ...
    <div>
      <nav className="flex items-center justify-between h-20 md:px-8 px-4 bg-linear-to-r from-white via-indigo-50 to-purple-50 border-b border-slate-200">
        ...
        <div className="flex justify-center items-center gap-5">
          ...
          <button
            onClick={() => {
              (removeFromSessionStorage("explainGrammer"),
                setExplainGrammerUpdate(!explainGrammer));
            }}
          ...
          <div className="relative rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:shadow-lg">
            <div className="flex items-center justify-between">
              ...
              <div className="flex justify-center items-center gap-5">
                ...
                <button
                  onClick={() => {
                    (setOpenExplain(false), setOpenRule(false));
                  }}
        ...
        {explainGrammer?.mistakes ? (
          <div className="md:px-6">
            <div className="flex justify-between items-center p-4 rounded-md bg-green-50 border border-green-200">
              <div className="flex justify-center items-center gap-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-green-500">
                    {explainGrammer?.correctedSentence}
                  </p>
                </div>
              </div>
              ...
              <div
                onClick={() =>
                  copyCorrectText(explainGrammer?.correctedSentence)
                }
                className="w-15 h-15 rounded-md flex justify-center items-center bg-green-100 border border-green-200 text-green-500 cursor-pointer">
                {copied ? <Check /> : <Copy size={23} />}
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 grid-cols-1 my-5 justify-center items-center gap-5">
              {explainGrammer?.mistakes.map((items) => (
                <div
                  className={`p-4 rounded-md ${changeIconOrColor(items?.type, "bg")} border`}
                  key={items?.id}>
                  <header className="flex justify-between items-center">
                    <div className="flex justify-center items-center gap-2">
                      <label
                        className={`w-10 h-10 rounded-full flex justify-center items-center border ${changeIconOrColor(items?.type, "char")} text-white shadow`}>
                        {items?.id}
                      </label>
                    ...
                    <div
                      className={`w-10 h-10 rounded-md flex justify-center items-center ${changeIconOrColor(items?.type, "bg")} border shadow`}>
                  ...
                  <div className="flex flex-col gap-1 mt-5">
                    <div
                      className={`flex justify-center items-center text-sm font-semibold ${changeIconOrColor(items?.type, "border")} border rounded-sm overflow-hidden`}>
                      <div
                        className={`${changeIconOrColor(items?.type, "correct")} py-1 px-5 pr-10`}>
                        Wrong
                      </div>
                    ...
                    <div
                      className={`flex justify-center items-center text-sm font-semibold ${changeIconOrColor(items?.type, "border")} border rounded-md overflow-hidden`}>
                      <div
                        className={`${changeIconOrColor(items?.type, "correct")} py-1 px-5 pr-10`}>
                        Correct
                      </div>
                    ...
                    <div
                      className={`flex justify-center items-center text-sm  ${changeIconOrColor(items?.type, "border")} border rounded-md overflow-hidden`}>
                      <div
                        className={`${changeIconOrColor(items?.type, "correct")} py-1  px-5 pr-10 font-semibold`}>
                        Explain
                      </div>
                      <div className="flex-1 py-1 px-5 bg-white flex justify-between items-center">
                        <span>{items?.explanation.slice(0, 25)}...</span>
                        <Eye
                          onClick={() => showRuleAndExample("explain", items)}
                          size={15}
                    ...
                    <div
                      ...
                      <div className="flex-1 py-1 px-5 bg-white flex justify-between items-center">
                        <span>{items?.rule.slice(0, 30)}...</span>
                        <Eye
                          size={15}
                          className="cursor-pointer"
                          onClick={() => showRuleAndExample("rule", items)}
                        />
                      </div>
                    </div>
                  </div>
            ...
            {loader ? (
              <div className="flex flex-col items-center gap-4 w-full max-w-xs mx-auto rounded-3xl border border-white/10 bg-white/10 px-10 py-8 shadow-2xl backdrop-blur-xl animate-pulse">
                ...
                <div className="space-y-1 text-center ">
                  ...
                  <p className="text-sm text-black/70">Please wait a moment</p>
                </div>
              </div>
            ) : error ? (
              ...
              <div className="flex flex-col justify-center items-center gap-2">
                ...
                <p className="mt-2 text-slate-500 max-w-md text-center">
                  Enter a sentence to identify grammar mistakes, understand why
                  they happened, and learn how to write it correctly.
                </p>
              </div>
            )}
          </div>
